...
def resolve_execution_providers(
    ...
    metadata = {
        ...
        "is_npu_accelerated": any(p in ("QNNExecutionProvider", "NNAPIExecutionProvider", "CoreMLExecutionProvider") for p in selected),
...
class PPOCRv4Detector:
    ...
    def postprocess(
        ...
        for c in contours[:self.max_candidates]:
            ...
            x1 = int(round(x1_640 * scale_x))
            y1 = int(round(y1_640 * scale_y))
            x2 = int(round(x2_640 * scale_x))
            y2 = int(round(y2_640 * scale_y))
...
class PPOCRv4Recognizer:
    ...
    def recognize_crop(self, crop_bgr: np.ndarray) -> Tuple[str, float]:
        ...
        if h <= 0 or w <= 0:
            return "", 0.0
        target_h = 48
        target_w = max(int(round(w * target_h / float(h))), 16)
