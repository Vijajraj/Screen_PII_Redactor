...
4. **End-to-End Latency**: Measured total loop latency of **559.3 ms** on CPU EP (**1.8 FPS** equivalent), comfortably fitting inside the recommended $500\,\text{ms}$ periodic scan refresh budget.
...
|---|---|---|---|---|
| **Screen Capture (`mss`)** | 2.75 ms | 5.45 ms | < 30 ms | **Optimal** |
| **Letterbox Preprocess** | 0.98 ms | 1.58 ms | < 10 ms | **Optimal** |
| **Model Inference (INT8 DBNet + SVTR)** | 553.00 ms | 626.91 ms | < 450 ms (CPU) | **Within Budget** |
| **Overlay & Blur Render** | 1.92 ms | 3.32 ms | < 10 ms | **Optimal** |
| **Total End-to-End Loop** | **559.34 ms** | **634.52 ms** | **< 500 ms** | **PASSED** |
...
- [x] **Capture-to-redaction latency measured** (559.3 ms on CPU EP)
