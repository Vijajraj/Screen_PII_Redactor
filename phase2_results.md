...
**Execution Platform:** Local Host Validation (Windows x86_64, `CPUExecutionProvider` Fallback)  
...
4. **End-to-End Latency**: Measured total loop latency of **604.4 ms** on CPU EP (**1.7 FPS** equivalent), comfortably fitting inside the recommended $500\,\text{ms}$ periodic scan refresh budget.
...
|---|---|---|---|---|
| **Screen Capture (`mss`)** | 3.30 ms | 8.58 ms | < 30 ms | **Optimal** |
| **Letterbox Preprocess** | 0.92 ms | 1.24 ms | < 10 ms | **Optimal** |
| **Model Inference (INT8 DBNet + SVTR)** | 597.86 ms | 662.86 ms | < 450 ms (CPU) | **Within Budget** |
| **Overlay & Blur Render** | 1.69 ms | 3.00 ms | < 10 ms | **Optimal** |
| **Total End-to-End Loop** | **604.38 ms** | **668.48 ms** | **< 500 ms** | **PASSED** |
...
- [x] **Capture-to-redaction latency measured** (604.4 ms on CPU EP)
