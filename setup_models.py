...
def download_and_convert():
    ...
    det_path = hf_hub_download(
        repo_id="Heliosoph/paddleocr-v4-det-onnx", filename="ch_PP-OCRv4_det.onnx", local_dir=MODELS_DIR
    )

    print("[2/4] Downloading PP-OCRv4 recognition ONNX model and dict...")
    hf_hub_download(repo_id="xberg-io/paddleocr-onnx-models", filename="rec/english/model.onnx", local_dir=MODELS_DIR)
    hf_hub_download(repo_id="xberg-io/paddleocr-onnx-models", filename="rec/english/dict.txt", local_dir=MODELS_DIR)
    ...
    d[1].dim_value = 3
    d[1].ClearField("dim_param")
    d[2].dim_value = 640
    d[2].ClearField("dim_param")
    d[3].dim_value = 640
    d[3].ClearField("dim_param")
    ...
    oq.quantize_dynamic(static_path, quantized_path, weight_type=oq.QuantType.QUInt8)
