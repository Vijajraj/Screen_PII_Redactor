...
# Title and Metadata
cells.append(nbf.v4.new_markdown_cell("""# Phase 1 — Model Build, Export & Local Validation
**Project:** Screen PII Redactor (On-Device Indian-Format PII, Snapdragon AI Lab Challenge)  
**Target Hardware:** Snapdragon Laptops (Hexagon NPU via QNNExecutionProvider)  
**Local Testbed:** Windows x86_64 Host (CPUExecutionProvider fallback)  
...
cells.append(nbf.v4.new_markdown_cell("""## Section 2: Load Pretrained OCR Detection & Recognition Models
Load the pretrained PP-OCRv4 Mobile DBNet text detector and SVTR recognizer.
"""))
cells.append(nbf.v4.new_code_cell("""# Check available model files in models/ directory
...
cells.append(nbf.v4.new_markdown_cell("""## Section 3: Synthetic Test Image Generation
...
"""))
cells.append(nbf.v4.new_code_cell("""from generate_synthetic_data import main as generate_data
generate_data()
"""))

# Section 4: PII Classification Layer
cells.append(nbf.v4.new_markdown_cell("""## Section 4: PII Classification Layer (Portable Stdlib + Regex)
...
"""))
cells.append(nbf.v4.new_code_cell("""from pii_classifier import classify_text, validate_verhoeff, validate_luhn
...
cells.append(nbf.v4.new_markdown_cell("""## Section 5: End-to-End Evaluation on Full Synthetic Test Set
Measure Precision, Recall, and False-Positive rate across all 20 synthetic images.
"""))
cells.append(nbf.v4.new_code_cell("""from evaluate_pipeline import evaluate_pii_classification
...
cells.append(nbf.v4.new_markdown_cell("""## Section 6: ONNX Model Export — Fixed Static Shape [1, 3, 640, 640]
...
"""))
cells.append(nbf.v4.new_code_cell("""m = onnx.load("models/detector_clean_static.onnx")
...
cells.append(nbf.v4.new_markdown_cell("""## Section 7: Post-Export Validation via ONNX Runtime (CPU EP)
Verify that the static ONNX model runs with zero shape-inference errors on CPUExecutionProvider.
"""))
cells.append(nbf.v4.new_code_cell("""sess = ort.InferenceSession("models/detector_clean_static.onnx", providers=["CPUExecutionProvider"])
...
cells.append(nbf.v4.new_markdown_cell("""## Section 8: Model Quantization (INT8) & Accuracy Drift
...
"""))
cells.append(nbf.v4.new_code_cell("""from evaluate_pipeline import evaluate_model_quantization
...
cells.append(nbf.v4.new_markdown_cell("""## Section 9: Execution Provider Abstraction Wrapper
...
"""))
cells.append(nbf.v4.new_code_cell("""from inference_wrapper import resolve_execution_providers, ScreenPIIPipeline
...
cells.append(nbf.v4.new_markdown_cell("""## Section 10: Phase 1 Deliverables Summary
...
"""))
cells.append(nbf.v4.new_code_cell("""deliverables = [
